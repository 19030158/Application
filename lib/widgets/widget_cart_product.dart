...
import 'package:dailypickings/utils/custom_stepper.dart';
import 'package:dailypickings/utils/utils.dart';
...
class CartProduct extends StatelessWidget {
  ...
  ListTile makeListTitle(BuildContext context) => ListTile(
    ...
    title: Padding(
      ...
      child: Text(
        //data.variationId == 0 ?
        data.productName, //: "${data.productName} (${data.attributeValue}${data.attribute}",
      style: TextStyle(color: Colors.black,
        fontWeight: FontWeight.bold),
      ),
      ),
    subtitle: Padding(
        padding: EdgeInsets.all(5), child: Wrap(direction: Axis.vertical,
    children: [
      Text("\$${data.productSalePrice.toString()}", style: TextStyle(
        color: Colors.black,
      ),
      ),
        FlatButton(
          ...
          onPressed: () {
            Utils.showMessage(context, "Daily Pickings", "Do you want to delete this item?","Yes",() {
              Provider.of<LoaderProvider>(context, listen: false).setLoadingStatus(true);
              var cartProvider = Provider.of<CartProvider>(context, listen:false).removeItems(data.productId);
              Provider.of<LoaderProvider>(context, listen: false).setLoadingStatus(false);

              Navigator.of(context).pop();
            }, buttonText2: "No", isConfirmationDialog: true, onPressed2: () {
              Navigator.of(context).pop();
            });
          },
